...
13. 敏感操作，如创建、删除虚拟机，重置密码等需要二次验证，APIKEY调用时则不需要
...
33. 系统兼容性实机测试采用“按影响范围同步”规则：仅当修改会直接影响以下任一内容时，才必须先阅读并评估是否同步修改 `scripts/check-system-compatibility.sh`、`server/compatibility_command.go`、`server/service/compatibility/`、`install.sh` 及对应文档：① 正式虚拟机创建链路（`CreateVMParams → CreateVM`）的参数语义、默认值、磁盘/网卡/显示/Guest Agent、CPU 拓扑、嵌套虚拟化、固件、机型、启动行为或 XML 注入；② x86_64/aarch64 架构配置及 KVM、QEMU、virt-install、libvirt 相关依赖或调用方式；③ 系统基础 OVS 网桥、网关、DHCP、NAT、转发规则、系统交换机、管理员默认安全组、VPC 绑定、Open vSwitch virtualport/vnet 端口创建或验证逻辑；④ 兼容性测试 CLI、脚本查找/下载/执行、报告、清理、安装继续/停止与回滚流程。只有变更实际触及上述测试输入、被测链路、判定条件或资源清理时才修改兼容性测试；纯前端样式与文案、普通接口文档、账户/认证/权限、监控统计、与上述链路无关的存储或网络功能、仅模板/导入流程以及不改变上述行为的重构和修复，均不需要阅读或修改兼容性测试相关文件。
34. 针对用户日志分析处理没有远程的情况下，你需要先从日志中判断问题，然后找到问题后先不要盲目写代码，先给我命令我发给用户进行测试后没有问题再进行代码修改，避免发版后问题仍然存在。