...
>
> **v0.18.2 修订要点（minisign.pub 独立公钥移除 + install.sh 语言环境检查对齐官方 + bash 审计脚本修复）**：
> 1. **移除独立 `minisign.pub` 公钥文件（与官方单一来源一致）**：删除 `docs/GCHSJ/minisign.pub`，并同步清理 `.github/workflows/build.yml` 中 5 处失效的 `release/minisign.pub` / `release-artifacts/minisign.pub` 引用（amd64/arm64 artifact、OSS 上传 ×2、GitHub Release 文件列表）——build.sh 自 v0.18 起已不再探测/分发公钥，验签公钥仅随 install.sh 内嵌 `MINISIGN_PUBLIC_KEY` 单一来源。同步校正 install.sh `extract_tarball` 验签注释与 build.sh 参数区注释（移除「docs/GCHSJ/minisign.pub 等探测文件」表述）。公钥明文仍以内嵌值随 install.sh 分发，签发流程不变（见 `docs/GCHSJ/minisign-publishing.md`）。
> 2. **`check_locale()` 语言环境检查对齐官方并放宽为可交互确认**：官方逻辑对非 `en_US.UTF-8|C.UTF-8|POSIX.UTF-8` 环境硬退出；本仓库改为同文案警告，交互模式额外提示 `read_tty -rp "是否仍以当前语言继续安装? [y/N]"`（超时 3s 默认 N 取消），非交互/CI 仅警告并以 `LANG=C.UTF-8` 继续，不阻断 CI。install.sh `bash -n` 语法校验通过。
> 3. **修复 install.sh 写入 `/root/.bashrc` 的审计脚本引号失衡**：`setup_bash_audit()` 原 `PROMPT_COMMAND` 行 `"` 未闭合、`$?` 未转义，导致每个新 shell `bash: rc=$?": bad substitution` 报错。已在 `install.sh` 源码修正（闭合引号 + `\$?` 转义），并对测试机 `/root/.bashrc`、`/etc/skel/.bashrc` 现网文件做字节级修复，`bash -n` 均通过。
>
> **v0.18.3 修订要点（链式克隆 backing 模板未纳入解锁集合 → SELinux 启动仍失败）**：
> 1. **实测定位**：v0.18.1 修复（`withVMDiskImagesUnlocked`）后，测试机链式克隆（`Ubuntu26.04-LTS` linked，`fix-test-02`）启动仍在 SELinux Enforcing 下报 `无法在 'system_u:object_r:virt_content_t:s0' 中设定安全上下文 '/opt/templates/Ubuntu26.04-LTS.qcow2': Operation not permitted`，即使运行二进制已含修复符号、`qemu-img` 可读、模板 immutable（`chattr +i`）完好。
> 2. **根因**：VM XML 的 `<source file>` 仅列出**覆盖层**（如 `/opt/images/fix-test-02.qcow2`），链式克隆的 immutable backing 模板路径**只存在于 qcow2 backing-file 元数据**中，`diskSourceFileRe` 无法从 XML 捕获模板 → 旧逻辑只解锁覆盖层，backing 模板依旧 immutable，libvirt 启动解析 backing 链时对该层 `setfilecon` 仍被 immutable 拦截。
> 3. **修复**：`withVMDiskImagesUnlocked` 对每个 `<source file>` 用新增 `resolveQcow2BackingChain(path)`（`qemu-img info -U` 递归解析 backing 链，含相对路径对父目录解析，上限 64 层）展开全部文件路径，逐层 `RemoveFileImmutable` → start → `defer SetFileImmutable` 恢复。实测：克隆启动成功（任务 `success`、`virsh domstate` 运行、guest-agent 响应），模板 immutable 恢复（`----i`）、模板 SELinux 上下文被 libvirt 正确打为 `virt_content_t`。已清理测试 VM。
> 4. **结论**：v0.18.1 修复在「覆盖层自身 immutable」场景有效，但未覆盖「backing 模板 immutable」的链式克隆主场景；v0.18.3 补足 backing-chain 解析后链式克隆启动完整通过。
>
> **v0.18 修订要点（HEAD 官方 GitHub 仓库核对比 + 测试机孤儿 VM 清理，代码零回归确认）**：
>
...
| **minisign 离线签名（§14.5 候选④，v0.12.1 已实施，v0.18.2 收敛公钥单一来源）** | §5.11.7 / §13.7.2 | ✅ **已实施（v0.12.1；v0.18.2 移除独立公钥文件）**：构建侧 build.sh 用 minisign 私钥（`MINISIGN_KEY`/`--minisign-key`，缺失降级跳过仅 SHA256）对 `.tar.gz` 签名产出 `.minisig`，**不探测/分发任何公钥文件**。安装侧 install.sh `extract_tarball` 解压前调 `verify_minisign_signature()`（须 `-V -m` 分开传参，`-Vm` 解析异常）——公钥**仅取内嵌 `MINISIGN_PUBLIC_KEY`**（单一来源，随 install.sh 分发）；有签名+公钥但验证失败 exit 1，无 minisign 命令/签名/公钥则降级 SHA256。v0.18.2 已删除 `docs/GCHSJ/minisign.pub` 与 CI 中 5 处 `release/minisign.pub` 引用（build.sh 早于 v0.18 即不再生成该文件）。密钥基建：离线私钥（`.minisign-sec/`，gitignore 不入库）+ 内嵌公钥，签发流程见 `docs/GCHSJ/minisign-publishing.md`，依赖登记 `docs/dependencies.md` |
