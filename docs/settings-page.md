...
| `security` | 安全与维护 | 开发环境开关、公网访问开关、SMTP 配置与测试发信、会话指纹/请求过滤/泄露密码检测、安全组默认全放通、JWT 密钥轮换、维护模式 |
...
- **公网访问**：默认关闭，开关使用风险确认弹窗；开启前后端检查全部有效管理员 2FA，并要求当前管理员新鲜 TOTP/恢复码验证。开启会撤销现有管理员 API Key；关闭公网访问时所有非局域网请求统一返回 403。详见 [`docs/public-access-security.md`](public-access-security.md)。
...
设置页使用以下接口（其中公网访问与安全初始化相关接口为本次新增或扩展）：
